...
若算法需要根据当前代码和本次参数准备运行入口，算法插件必须提供明确规则和验证方法。框架按规则自动准备或调整本次 run 的工作副本，并在正式批量运行前验证入口可运行；验证失败时要求用户修正规则或算法接入内容。整个过程不修改算法源码仓库或数据集原始文件，框架也不自行猜测算法语义。
...
对外提供数据集适用性检查、隔离工作区准备、算法运行、输出验证和过程信息采集能力。原始传感器、标定和参考文件始终只读；首次录入或显式刷新时，只允许在每个具体数据集根目录创建或原子更新系统生成的实例 YAML。
...
6. 单个 Segment 的算法运行失败时记录失败，默认继续当前数据集的后续 Segment；全部 Segment 处理后，算法失败次数超过本次阈值则判定 run 失败，否则判定成功。
...
| 输入类 | 默认模式将问题数据集记为未运行并继续；首次失败退出模式保存诊断后终止 | 否 | 保留此前完成的数据集结果 |
| 基础设施类 | 无法继续时暂停或终止后续阶段；条件恢复且快照未变化时允许继续 | 否 | 保留全部已确认事实和检查点 |
| 算法运行类 | 默认模式记录当前失败 Segment 并继续后续 Segment；首次失败退出模式立即终止 | 是 | 保留失败证据和其他 Segment 结果 |
| 评估类 | 保留算法原始输出，将该数据集标为未评估并继续；修复后可追加新的评估快照，不覆盖原失败记录 | 否 | 保留运行结果及评估诊断 |

失败处理模式随本次 run 冻结。默认模式用于无人值守批量运行；首次失败退出模式由用户手动启用，用于在第一个数据集预检或算法运行问题处停止并人工排查。用户主动中断和无法保存事实的基础设施错误不受模式控制，始终安全停止。
...
- 将算法输出等派生产物迁移到 run 的独立工作区，保证算法仓库与数据集原始文件不被 Pipeline 改写；每个具体数据集根目录只保留系统生成的实例 YAML。
...
| 数据集如何确认身份和内容未变化，同时保持原始文件只读？ | 数据集、恢复、可比性 | LLD |
